	...
	public void onClick(View v) {
		...
		switch (v.getId()) {
			case R.id.button_id:
				...
				break;
			case R.id.button1:
				if(!mDsObjectUtils.isEmpty(mEditTextId.getText().toString()) && is_idcheck && !mDsObjectUtils.isEmpty(mEditTextPw.getText().toString())&& !mDsObjectUtils.isEmpty(mEditTextName.getText().toString())){
					...
					if(mDsObjectUtils.isEmpty(mEditTextId.getText().toString())){
						...
						Toast.makeText(getApplicationContext(), "ID 중복 체크 해주시기 바랍니다.", Toast.LENGTH_SHORT).show();
						...
						Toast.makeText(getApplicationContext(), "이름을 입력 해주시기 바랍니다.", Toast.LENGTH_SHORT).show();
					}
